...
//==============SE ACTUALIZA EL DESCUENTO SEGUN LA BASE DE DATOS================
$.ajax({
	method: "POST",
	...
	success:function(respuesta){
		...
		$( ".oferta" ).html(respuesta[0].sms_promocion);
			...
			$('.ir_carrito').on('click',function(e){
			...
			$(".cantidad_cesta").html(listaCarrito.length);//se escribe la sesta del carrito
			...
			var cantidad_cesta= Number($(".cart-notification").html());//traigo lo q tiene la cesta
		...
		if(localStorage.getItem("cantidadCesta")!=null){//viene con informacion
			...
			//5.3
			$(".cart-notification").html(localStorage.cantidadCesta);
